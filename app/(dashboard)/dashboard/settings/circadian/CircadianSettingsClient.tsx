...
export default function CircadianSettingsClient({ initial }: { initial: { selfId: 'morning'|'neither'|'evening'; wakeTime: string; bedtime: string; shiftWork: boolean } }) {
  ...
  const submit = async () => {
    setPending(true)
    const r = await fetch('/api/circadian/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    const body = await r.json().catch(() => ({}))
    if (!r.ok) {
      toast({ title: 'Update failed', description: String(body?.error ?? 'Error'), variant: 'destructive' })
      setPending(false)
      return
    }
  ...
  return (
    <div className="container mx-auto max-w-xl py-8 space-y-4">
      <h1 className="text-2xl font-semibold">Circadian Profile</h1>
      ...
      <div className="flex justify-end gap-2">
        <button className="px-3 py-2 rounded bg-neutral-900 text-white disabled:opacity-50" disabled={pending} onClick={submit}>{pending ? 'Saving…' : 'Update Circadian Profile'}</button>
      </div>
    </div>
  )
}
